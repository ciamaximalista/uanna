...
- User panel organized as a menu of independent views for profile, notifications, network, timeline search, private messages, export/migration, and app download.
...
- On-demand social graph PNG for the node, generated by the administrator and downloadable from each user's network panel.
- `Connected with...` network view listing remote profiles followed by members of the node, ordered by number of local followers, with follow, unfollow, mute, and block controls.
